        ...
        try {
            ...
            const setId = setInterval(()=>{
                ...
                if(현재시간>시작시간){
                    ...
                    시 = Math.floor(남은시간/(60*60*1000)) % (24*state.타임세일기한);
                ...
                setTimer({
                    시: String(시).padStart(2, '0'),  /* 시 < 10 ? `0${시}` : 시, */
                    분: String(분).padStart(2, '0'),  /* 분 < 10 ? `0${분}` : 분, */
                    초: String(초).padStart(2, '0')  /* 초 < 10 ? `0${초}` : 초 */ 
                })
            },1000);
                                    ...
                                    return(
                                        <li className={`slide slide${idx+2}`} key={item.상품번호}>
                                            <a href="!#" onClick={(e)=>onClickViewProduct(e, item, `./images/main/${sectionName}/${item.상품이미지}`)}>
                                                ...
                                                {
                                                    item.카테고리==="건강식품" &&
                                                    <em><img src={`./images/main/${sectionName}/pick1.png`} alt="" /></em>
                                                }    
                                                </div>
